...
/**
 * All Gemini prompt templates for ACM.
 */

export const CLASSIFIER_SYSTEM_PROMPT = `You are a senior software engineer analyzing Git commits.
...
export function buildGrouperPrompt(
  ...
  return `You are a senior software engineer analyzing a developer's commit history.
...
export function buildReporterPrompt(
  period: string,
  workUnits: Array<{ name: string; type: WorkType; commitCount: number }>,
  stats: {
  ...
  return `You are writing a professional developer work summary for the period: ${period}.
...
export function buildQueryPrompt(question: string, context: string): string {
  return `You are ACM, a personal developer productivity assistant.
